...
def game_board(game_map, player=0, row=0, column=0, just_display=False):
    try:
        if game_map[row][column] != 0:
            print("This position is occupied! Pick another!")
        ...
        print("Something went wrong!, check", e)
        return game_map, False
...
while play:
    ...
    try:
        game_size = int(input("Size of tic tac toe game? (2 - 7) "))
    except ValueError:
        game_size = 0
    if not game_size in range(2, 8):
        game_size = 3
        print("Invalid size! Game set to 3x3 tic tac toe.")
    ...
    if game_size in range(2, 5):
        ...
        if not max_players in range(2, game_size - 1):
            max_players = 2
            print("Can't play with that no. of players. Default set as 2 players.")
